...
\begin{derivation}
\paragraph{Where $\mathbf{Q}_i\dd_i = \M_{ii}(\vv_i^+-\vv_C)$ comes from.}
At node $i$, the converged system $\LL\vv^+=\B$ reads $\M_{ii}\vv_i^+ + (\C\vv^+)_i = \B_i$. Substituting
$\B_i = \tilde\bb_i - \mathbf{P}_i\f_i + \M_{ii}\mathbf{P}_i\vv_C$ and $\tilde\bb_i = \f_i + (\C\vv^+)_i$, the
$(\C\vv^+)_i$ terms cancel, leaving
$$
\M_{ii}\vv_i^+ = \mathbf{Q}_i\f_i + \M_{ii}\mathbf{P}_i\vv_C.
$$
Hence $\mathbf{Q}_i\f_i = \M_{ii}(\vv_i^+-\mathbf{P}_i\vv_C)$, and using $\mathbf{Q}_i\dd_i=\mathbf{Q}_i\f_i-\M_{ii}\mathbf{Q}_i\vv_C$ together with $\mathbf{P}_i+\mathbf{Q}_i=\I$:
\begin{equation}
    \boxed{\mathbf{Q}_i\dd_i = \M_{ii}(\vv_i^+-\vv_C)}
\end{equation}
\end{derivation}
...
$$
unlike a plane, where $\n_i$ is a constant. The collider may translate at a constant velocity
$\vv_C$ (treated here as a fixed constant, not yet a function of $\x_i^-$ — see the rotating-collider
extension for that case). At an active contact the frictionless contact force is
$\rr_i = -d_n\,\n_i$ with $d_n := \n_i^\top\dd_i$, $\dd_i := \f_i - \M_{ii}\vv_C$, and
$\f_i := \tilde\bb_i - (\C\vv^+)_i$, so the effective per-node right-hand side actually solved for is
$$
\B_i = \tilde\bb_i + \rr_i = \tilde\bb_i - \mathbf{P}_i\f_i + \M_{ii}\mathbf{P}_i\vv_C, \qquad \mathbf{P}_i := \n_i\n_i^\top.
$$
...
$$
\newline \textbf{Contracting with the adjoint.} With $\vv_C$ constant, the extra contribution to
$\pdc{\phi}{\x_i^-}$ from this channel is $-\z_i^\top(\delta\mathbf{P}_i\,\dd_i)$ (as before, but with
$\f_i$ replaced by $\dd_i=\f_i-\M_{ii}\vv_C$, since $\B_i$'s $\mathbf{P}_i$-dependent part is now
$-\mathbf{P}_i\dd_i$). Using symmetry of $\mathbf{Q}_i$ to move it onto $\z_i$ and onto $\dd_i$ in turn,
\begin{align*}
\z_i^\top\,\delta\mathbf{P}_i\,\dd_i
&= \frac1{r_i}\Big[(\n_i^\top\dd_i)\,(\mathbf{Q}_i\z_i)^\top + (\n_i^\top\z_i)\,(\mathbf{Q}_i\dd_i)^\top\Big]\delta \\
&= \frac1{r_i}\Big[d_n\,\z_i^\perp + (\n_i\cdot\z_i)\,\mathbf{Q}_i\dd_i\Big]^\top\delta
\end{align*}
where $\z_i^\perp = \mathbf{Q}_i\z_i$ is exactly the contact-split perpendicular component defined
earlier.
\newline \textbf{Eliminating $\mathbf{Q}_i\dd_i$.} By the moving-collider identity derived above,
$\mathbf{Q}_i\dd_i = \M_{ii}(\vv_i^+-\vv_C)$ holds exactly at active contacts (reducing to
$\mathbf{Q}_i\f_i = m_i\vv_i^+$ when $\vv_C=\zero$).
...
\begin{equation}
    ...
    = -\frac1{r_i}\Big[\,d_n\,\z_i^\perp \;+\; (\n_i\cdot\z_i)\,m_i(\vv_i^+-\vv_C)\,\Big]
    }
\end{equation}
added on top of the base formula, for every particle $i$ with an active contact; particles
with no active contact, or contacts against a flat plane, receive no correction. Setting
$\vv_C=\zero$ recovers the static-collider result exactly.
\end{derivation}
...
$$
As before, $\mathbf{P}_i=\n_i\n_i^\top$, $\mathbf{Q}_i=\I-\mathbf{P}_i$, $\f_i=\tilde\bb_i-(\C\vv^+)_i$, and,
with the collider translating at constant velocity $\vv_C$, $\dd_i=\f_i-\M_{ii}\vv_C$, $d_n=\n_i^\top\dd_i$.
The collider-agnostic results
$$
\Delta\left(\pd\phi{\x_i^-}\right) = -\Big[d_n\,\mathbf S_i\z_i+(\n_i\cdot\z_i)\,\mathbf S_i\dd_i\Big]
\quad \mathbf S_i:=\pd{\n_i}{\x_i^-}
$$
...
$\mathbf a\cdot\dd_i=\mathbf a\cdot(\mathbf{Q}_i\dd_i)=m_i\,\mathbf a\cdot(\vv_i^+-\vv_C)$, so:
\begin{gather*}
\mathbf S_i\z_i = \frac1{\rho_i}\big(\z_i^\perp-(\mathbf a\cdot\z_i)\mathbf a\big)=\frac1{\rho_i}\mathbf{P}_\perp\z_i^\perp
\\[1ex]
\mathbf S_i\dd_i = \frac{m_i}{\rho_i}\Big[(\vv_i^+-\vv_C)-\big(\mathbf a\cdot(\vv_i^+-\vv_C)\big)\mathbf a\Big]=\frac{m_i}{\rho_i}\mathbf{P}_\perp(\vv_i^+-\vv_C)
\end{gather*}
\newline \textbf{Result.}
\begin{equation}
    \boxed{
...
\end{equation}
i.e.\ exactly the sphere formula with $r_i\to\rho_i$ (distance to the \emph{axis}), projected onto the plane orthogonal to $\mathbf a$, motion along the axis never changes the normal, so it contributes nothing. Setting $\vv_C=\zero$ recovers the static-collider result exactly.
\end{derivation}
...
$\Delta(\pdc\phi{\x_i^-})=-\z_i^\top(\delta\mathbf P_i\dd_i)$ never used anything about $\mathbf S_i$ except
...
$$
\newline \textbf{Result.} Substituting the already-derived collider-specific values of $\mathbf S_i\z_i,\mathbf S_i\dd_i$
...
zero for a plane ($\mathbf S_i=\zero$) or an inactive contact. Setting $\vv_C=\zero$ recovers the
static-collider result exactly.
\end{derivation}

\begin{derivation}
\paragraph{Correction to $\pdc{\phi}{\x_i^-}$ due to Spherical Collider Rotation} \mbox{}\par\medskip
At contact point $i$ whe have the folloiwng:
\begin{equation*}
    \B_i = \tilde\bb_i - \mathbf{P}_i\f_i + \M_{ii}\mathbf{P}_i\vv_C
\end{equation*}
Now the collider velocity $\vv_C$ is a function of the contact point $\x_i^-$, so we have to differentiate it as well. We have:
\begin{equation*}
    \pd{\B_i}{\x_i^-} = 
        \pd{\tilde\bb_i}{\x_i^-} - 
        \pd{\mathbf{P}_i}{\x_i^-}\f_i - 
        \mathbf{P}_i\pd{\f_i}{\x_i^-} + 
        \M_{ii}\pd{\mathbf{P}_i}{\x_i^-}\vv_C + 
        \M_{ii}\mathbf{P}_i\pd{\vv_C}{\x_i^-}
\end{equation*}
The loss gradient with respect to the previous-step position $\x_i^-$ is then:
\begin{gather*}
\pd{\phi}{\x_i^-} = \z_i^\top\pd{\B_i}{\x_i^-} = \\
    \underbrace{\z_i^\top\left(\pd{\tilde\bb_i}{\x_i^-} - \mathbf{P}_i\pd{\f_i}{\x_i^-}\right)}_{h\Delta\LL\z^\perp - \frac1h\C\z^\perp} - 
    \underbrace{\z_i^\top\left(\pd{\mathbf{P}_i}{\x_i^-}\f_i - \M_{ii}\pd{\mathbf{P}_i}{\x_i^-}\vv_C\right)}_{\Delta\left(\pdc{\phi}{\x_i^-}\right)} +
    \underbrace{\z_i^\top\left(\M_{ii}\mathbf{P}_i\pd{\vv_C}{\x_i^-}\right)}_{\Delta^\circlearrowleft\left(\pdc{\phi}{\x_i^-}\right)}
\end{gather*}
So in case of collider velocity being a function of the contact point, we have an additional term in the loss gradient with respect to the previous-step position $\x_i^-$:
\begin{equation}
    \boxed{
    \Delta^\circlearrowleft\left(\pd{\phi}{\x_i^-}\right) = \z_i^\top\left(\M_{ii}\mathbf{P}_i\pd{\vv_C}{\x_i^-}\right)
    }
\end{equation}
...
\begin{derivation}
\paragraph{Spherical Collider contact-point velocity as a composed function $\vv_C = \f(\g(\x_i^-))$, and its curvature correction.} \mbox{}\par\medskip
For a rotating sphere collider, decompose the contact-point velocity into a surface-point map $\g$
and a rigid-body point-velocity map $\f$, both evaluated at the current time $t$:
\begin{equation*}
    \g(\x_i^-) = \mathbf{o} + R\,\n_i(\x_i^-) 
    \quad\quad 
    \f(\mathbf{y}) = \vv_t + \boldsymbol\omega \times (\mathbf{y} - \p)
\end{equation*}
so that $\vv_C = \f(\g(\x_i^-))$. Their Jacobians, with $r_i := \lVert\x_i^- - \mathbf{o}\rVert$:
\begin{equation*}
    \pd{\g}{\x_i^-} = \frac{R}{r_i}\mathbf{Q}_i
    \qquad\qquad
    \pd{\f}{\mathbf{y}} = [\boldsymbol\omega]_\times
\end{equation*}
By the chain rule:
\begin{equation}
    \boxed{
    \pd{\vv_C}{\x_i^-} = \pd{\f}{\g}\pd{\g}{\x_i^-} = \frac{R}{r_i}[\boldsymbol\omega]_\times\,\mathbf{Q}_i
    }
\end{equation}
Substituting this into $\Delta^\circlearrowleft\left(\pdc{\phi}{\x_i^-}\right) = \z_i^\top\left(\M_{ii}\mathbf{P}_i\pdc{\vv_C}{\x_i^-}\right)$ gives the explicit rotation correction:
\begin{equation}
    \boxed{
    \Delta^\circlearrowleft\left(\pd{\phi}{\x_i^-}\right) = \frac{\M_{ii}\,R}{r_i}\,\z_i^\top\mathbf{P}_i[\boldsymbol\omega]_\times\mathbf{Q}_i
    }
\end{equation}
\end{derivation}

\begin{derivation}
\paragraph{Collider contact-point velocity as a composed function $\vv_C = \f(\g(\x_i^-))$ for a Capsule, and its curvature correction.} \mbox{}\par\medskip

\begin{reminder}
Parametrize the capsule by segment endpoints $\mathbf p_0,\mathbf p_1$, unit axis $\mathbf a = (\mathbf p_1-\mathbf p_0)/L$, radius $R$.
With $t(\x) := \mathrm{clamp}\big(\mathbf a\cdot(\x-\mathbf p_0),\,0,\,L\big)$, the closest point on the segment is
$\mathbf c(\x) = \mathbf p_0 + t(\x)\,\mathbf a$, and the surface map is $\g(\x) = \mathbf c(\x) + R\,\n(\x)$,
$\n(\x) := (\x-\mathbf c(\x))/\rho(\x)$.
\end{reminder}

\textbf{Cap regime} ($t\in\{0,L\}$). 
The result is exactly the sphere:
\begin{equation}
    \boxed{
    \Delta^\circlearrowleft\left(\pd{\phi}{\x_i^-}\right) = \frac{\M_{ii}\,R}{r_i}\,\z_i^\top\mathbf{P}_i[\boldsymbol\omega]_\times\mathbf{Q}_i
    }
    \qquad \text{(cap regime)}
\end{equation}

\textbf{Cylindrical regime} ($0<t<L$). Now $\mathbf c(\x)$ itself slides
along the axis as $\x_i^-$ moves, $\pdc{\mathbf c}{\x_i^-} = \mathbf a\mathbf a^\top$ (\emph{not} zero as in
the cap regime), so
\begin{equation*}
    \pd{\g}{\x_i^-} = \mathbf a\mathbf a^\top + R\,\mathbf S_i, \qquad
    \mathbf S_i := \pd{\n_i}{\x_i^-} = \frac{1}{\rho_i}\big(\mathbf{Q}_i-\mathbf a\mathbf a^\top\big)
\end{equation*}
Unlike the static (translation-only) curvature correction — which only ever differentiates $\n_i$, and
so is killed along the axis by $\mathbf S_i\mathbf a=\zero$ — this extra $\mathbf a\mathbf a^\top$ term
does \emph{not} vanish here: composing with $\pdc\f{\mathbf y}=[\boldsymbol\omega]_\times$ gives
\begin{equation*}
    \pd{\vv_C}{\x_i^-} = [\boldsymbol\omega]_\times\Big(\mathbf a\mathbf a^\top + \frac{R}{\rho_i}(\mathbf{Q}_i-\mathbf a\mathbf a^\top)\Big)
\end{equation*}
Substituting into $\Delta^\circlearrowleft\left(\pdc{\phi}{\x_i^-}\right) = \z_i^\top\left(\M_{ii}\mathbf{P}_i\pdc{\vv_C}{\x_i^-}\right)$:
\begin{gather}
\boxed{
\begin{gathered}
\mathbf c_i := \boldsymbol\omega\times\n_i \qquad
\mathbf c_{i,\parallel} := \mathbf a(\mathbf a\cdot\mathbf c_i) \qquad
\mathbf c_{i,\perp} := \mathbf c_i-\mathbf c_{i,\parallel}
\\[6pt]
\Delta^\circlearrowleft\left(\pd{\phi}{\x_i^-}\right) = -\M_{ii}\,(\n_i\cdot\z_i) \left[ \mathbf c_{i,\parallel} + \frac{R}{\rho_i}\,\mathbf c_{i,\perp} \right]
\end{gathered}
}
\end{gather}
\end{derivation}
